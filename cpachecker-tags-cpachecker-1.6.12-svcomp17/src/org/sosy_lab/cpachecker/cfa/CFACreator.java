...
private boolean classifyNodes = false;
  ...
  public CFACreator(Configuration config, LogManager logger, ShutdownNotifier pShutdownNotifier)
    ...
    if(MyInfo.parser==null) {
      MyInfo.parser=parser;
    }
...
v.addInitializer(initializer);
  ...
  private void exportCFA(final CFA cfa) {
    ...
    // write CFA to file
    Path myexportCfaFile = Paths.get("output\\"+MyInfo.pcName+".dot");
    if (exportCfa && exportCfaFile != null) {
      try (Writer w = MoreFiles.openOutputFile(myexportCfaFile, Charset.defaultCharset())) {
  ...
  public void setMainFunctionName(String pFunc) {
    mainFunctionName = pFunc;
