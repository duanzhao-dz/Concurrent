...
public class CPAchecker {
  ...
  @Option(
      secure = true,
      name = "analysis.useErrorInterp",
      description="use error interpolant")
  private boolean useErrorInterp=false;

  @Option(
      secure = true,
      name = "analysis.useSafetyInterp",
      description="use Safety interpolant")
  private boolean useSafetyInterp=false;

  @Option(
      secure = true,
      name = "analysis.useWeight",
      description="use Weight")
    ...
    try {
      try {
        ...
        if (runCBMCasExternalTool) {
          ...
          MyInfo.pcName=programDenotation;
          cfa = parse(programDenotation, stats);
//          int i=1;
//          if(i==1){
//            return null;
//          }
         // SymInfo.findUselessEdges();
          PrincessInfo.setOperator();
  ...
  private CFA parse(String fileNamesCommaSeparated, MainCPAStatistics stats) throws InvalidConfigurationException, IOException,
    ...
    MyInfo.logger=logger;
    MyInfo.shutdownNotifier=shutdownNotifier;
