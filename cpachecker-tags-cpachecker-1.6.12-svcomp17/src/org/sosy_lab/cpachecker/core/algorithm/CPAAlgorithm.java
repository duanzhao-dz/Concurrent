...
public class CPAAlgorithm implements Algorithm, StatisticsProvider {
  ...
  private Map<AbstractState, BooleanFormula> stateToFor = new HashMap<AbstractState, BooleanFormula>(
      2);
  private AlgorithmStatus run0(final ReachedSet reachedSet) throws CPAException, InterruptedException {
    ...
    while (reachedSet.hasWaitingState()) {
      ...
      SymInfo.sucSameE=false;
      SymInfo.sucSameS=false;
      ...
      CFANode loc = ((ARGState) state).getLoc();
      if (loc == null) {
        loc = AbstractStates.extractLocation(state);
        ((ARGState) state).setLoc(loc);
      }
     // loc.setKey(-1);



       int stateId=((ARGState)state).getStateId();
//       if(stateId==68912){
//         int i=1;
//         i=i+1;
//       }
      System.out.println(stateId+"@N");
////
  ...
  private boolean handleState(
    ...
    for (Iterator<? extends AbstractState> it = successors.iterator(); it.hasNext();) {
      ...
      if (((ARGState) successor).isDestroyed()) {
        continue;
      }
      SymInfo.tmpCond=0;
      ...
      if (stop) {
        ...
        stats.addTimer.stop();
        nextState = successor;
        stateToFor.put(successor, SymInfo.curFor);
//        if(((ARGState)state).getMayCE()!=null){
//          ((ARGState)successor).setMayCE(((ARGState)state).getMayCE());
//          ((ARGState)successor).setSub(((ARGState)state).getSub()+1);
//        }
      }
    }
