...
public class SequentialInterpolation<T> extends ITPStrategy<T> {
  ...
  public List<BooleanFormula> getInterpolants(
    ...
    for (int end_of_A = 0; end_of_A < formulas.size() - 1; end_of_A++) {
      ...
      BooleanFormula intp = getInterpolantFromSublist(interpolator.itpProver, formulas, start_of_A, end_of_A);
      interpolants.add(intp);
      if (SymInfo.lastFalse == -1 && intp.toString().equals("false")) {
        SymInfo.lastFalse = end_of_A;
      }
    }
    return interpolants;
  }
