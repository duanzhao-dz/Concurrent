...
public class AbstractARGBasedRefiner implements Refiner, StatisticsProvider {
  ...
  public final boolean performRefinement(ReachedSet pReached) throws CPAException, InterruptedException {
    ...
    if(SymInfo.errorPath.size()!=0){
      if(SymInfo.errorPath.contains(SymInfo.sb.toString())){
        UnmodifiableIterator<ARGState> it = path.getStateSet().iterator();
        while(it.hasNext()){
          ARGState next = it.next();
          next.setDonotCover(true);
        }
        return true;
      }
    }
    ...
    if (!counterexample.isSpurious()) {
      SymInfo.errorPath.add(SymInfo.sb.toString());
